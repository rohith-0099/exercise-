...
const mainContentArea = document.getElementById('main-content-area'); // New reference for main content area
...
playPauseBtn.addEventListener('click', () => {
    ...
    playPauseBtn.classList.add('animate-press');
    playPauseBtn.addEventListener('animationend', () => {
        playPauseBtn.classList.remove('animate-press');
    }, { once: true });
...
function updateProgressBar() {
    ...
    progressBarContainer.style.setProperty('--thumb-position', `${progress}%`);
...
// Show/hide progress thumb on hover
progressBarContainer.addEventListener('mouseover', () => {
    progressThumb.classList.remove('opacity-0');
});

progressBarContainer.addEventListener('mouseout', () => {
    progressThumb.classList.add('opacity-0');
});
...
function playSong(title, artist, albumArtUrl) {
    ...
    if (!isPlaying) { // Automatically play if not already playing
        playPauseBtn.click(); // Trigger the click event to start playing
    } else {
        clearInterval(progressInterval); // Clear existing interval if already playing
        isPlaying = false; // Temporarily set to false to allow click to re-trigger
        playPauseBtn.click(); // Re-trigger the click event to play new song
    }
    updateProgressBar(); // Ensure progress bar updates immediately
}

// Function to load content based on sidebar selection with section transition
function loadSection(sectionId) {
    // Apply exit animation classes
    mainContentArea.classList.remove('animate-enter');
    mainContentArea.classList.add('opacity-0', 'translate-y-4');

    // Wait for exit animation to complete before changing content and applying enter animation
    setTimeout(() => {
        ...
        mainContentArea.classList.remove('opacity-0', 'translate-y-4');
        mainContentArea.classList.add('animate-enter');
    }, 300); // Duration matches transition-all duration-500 in HTML for smoother transition
}
